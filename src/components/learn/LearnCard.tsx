          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                const correctText = (card.options || [])[correct as number] ?? "";
                const wasRight = i === correct;
                const payload = wasRight
                  ? `[LEARN_ANSWER] type=mcq result=correct chosen="${chosenLetter}) ${opt}"`
                  : `[LEARN_ANSWER] type=mcq result=incorrect chosen="${chosenLetter}) ${opt}" correct="${correctLetter}) ${correctText}"`;
                setTimeout(() => onAnswer?.(payload), 350);
              }}
      ...
      {!submitted ? (
        <button
          ...
          onClick={() => {
            ...
            const correctText = Array.from(correctSet).map((i) => `"${opts[i as number]}"`).join(", ");
            const isAllRight = picks.length === correctSet.size && picks.every((p) => correctSet.has(p));
        ...
        {[
          ...
          return (
            <button
              ...
              onClick={() => {
                setPicked(b.val);
                const wasRight = b.val === card.correct;
                const payload = wasRight
                  ? `[LEARN_ANSWER] type=truefalse result=correct chosen="${b.label}"`
                  : `[LEARN_ANSWER] type=truefalse result=incorrect chosen="${b.label}" correct="${card.correct ? tt.correct : tt.wrong}"`;
                setTimeout(() => onAnswer?.(payload), 350);
              }}
        ...
        {!sent && (
          <button
            ...
            onClick={() => {
              ...
              const correctAns = String(card.answer || "").trim();
              const wasRight = entered.toLowerCase() === correctAns.toLowerCase();
              const payload = wasRight
                ? `[LEARN_ANSWER] type=fill result=correct entered="${entered}"`
                : `[LEARN_ANSWER] type=fill result=incorrect entered="${entered}" correct="${correctAns}"`;
              setTimeout(() => onAnswer?.(payload), 350);
            }}
    ...
    <CardShell tone="violet" label={tt.match_columns}>
      ...
      {!done ? (
        <button
          ...
          onClick={() => {
            setDone(true);
            const results = pairs.map((p: any, i: number) => {
